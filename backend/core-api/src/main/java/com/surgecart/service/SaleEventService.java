...
public class SaleEventService {
    ...
    /**
     * A sale can already be LIVE in PostgreSQL when the app starts (the dev
     * seed data, or a restart mid-sale with an emptied Redis). Without its
     * stock key, reserve.lua reports every claim as "sale not live". Only
     * seeds keys that are missing, so a restart never resets live stock.
     */
    @EventListener(ApplicationReadyEvent.class)
    public void seedLiveSalesOnStartup() {
        for (SaleEvent sale : saleEventRepository.findByStatusOrderByStartsAtDesc(Enums.SaleStatus.LIVE)) {
            redisTemplate.opsForValue().setIfAbsent(
                    "sale:" + sale.getId() + ":stock", String.valueOf(sale.stockRemaining()));
        }
    }
