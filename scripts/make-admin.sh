#!/usr/bin/env bash
# Promotes a registered user to ADMIN.  ./scripts/make-admin.sh your@email.com
# Log out and back in afterwards - the role lives inside the JWT.
set -e
cd "$(dirname "$0")/.."   # docker compose runs next to docker-compose.yml
[ -z "$1" ] && { echo "Usage: ./scripts/make-admin.sh your@email.com"; exit 1; }
