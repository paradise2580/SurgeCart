...
load. See `core-api/src/main/java/com/surgecart/service/{Naive,Optimistic,
...
Two honest notes before the numbers: everything below was run inside this
project's build/development sandbox — a single shared vCPU container, not
dedicated server hardware, and not a Testcontainers/CI environment. Treat
absolute throughput numbers as **directionally real, not production
capacity planning numbers**. Re-run `loadtest/reserve-load-test.js` against
your own deployed infrastructure for numbers that matter for your resume.

Second: **the Java/Spring Boot side was written but not compiled or
executed in this sandbox.** The sandbox's network egress allowlist blocks
Maven Central (`repo.maven.apache.org` returns `host_not_allowed`), so
`mvn verify` could not run here. The Go gateway and the Angular client,
by contrast, both actually built, ran, and were load-tested against a
real local PostgreSQL/Redis in this same sandbox — see below. Before you
claim any Java-side number on a resume, run this yourself:
...
20000 operations in 0.903s -> ~22,100 ops/sec (single shared vCPU, this sandbox)
```

### 4.2 Java gateway — expected shape, unverified numbers
...
Fill this table in yourself once you've run `mvn verify` locally — do not
present these as measured numbers in an interview without having run them.
...
(Java, unverified in-sandbox — see section 4).
